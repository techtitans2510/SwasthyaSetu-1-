import { findRoutesByQuery, getRoutesForRole } from "../services/navigationRegistry";
...
export async function askAiAssistant({ message, role = "public", currentPage = "/", currentSection = "General", language = "en" }) {
  ...
  try {
    ...
    console.warn("[ai.api.js] Backend /api/ai/navigate unreachable, using client fallback:", networkError.message);
...
function clientSideFallback(query, role, language) {
  ...
  const isDocLookup = /(dawa parchi|parchi dikhao|purani parchi|view prescription|records|reports|ehr|lab report|blood report|discharge summary)/i.test(query);
  const isMedQuery = !isDocLookup && /(prescrib|diagnos|what disease|kya bimari|cure for|remedy|treatment|symptom|chest pain|fever for|difficulty breathing|paracetamol|crocin|azithromycin|amoxicillin|dosage|kitni goli|dawa batao|dawa suggest|dawai batao|ilaj batao)/i.test(query);
  ...
  if (isMedQuery) {
    ...
    const fallbackActions = (normRole === "asha")
      ? [
          { label: "Create Hospital Referral", routeKey: "asha_new_referral", route: "/worker/referrals/new", isPrimary: true },
          { label: "Scheduled Home Visits", routeKey: "asha_visits", route: "/worker/visits", isPrimary: false }
        ]
      : [
          { label: "Book Doctor Appointment", routeKey: "patient_appointments", route: "/appointments", isPrimary: true },
          { label: "Find Healthcare Facilities", routeKey: "patient_facilities", route: "/facilities", isPrimary: false }
        ];
  ...
  }[language] || `I found **${primary.label}** for your request.\n\n${primary.shortDescription}`;
