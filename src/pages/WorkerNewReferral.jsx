  ...
  return (
    <div className="worker-dashboard">
      ...
      >
        ...
        {selectedPatientId ? "Back to Patient Profile" : "Back to Referrals"}
      </Link>

      <header className="worker-page-header">
        <div>
          <span className="worker-eyebrow">Care Escalation</span>
          <h1>Generate Facility Referral</h1>
          <p>
            Connect community patients to Primary Health Centres, Community Health Centres, or District Hospitals.
          </p>
        </div>
      </header>
      ...
      <div className="worker-panel" style={{ maxWidth: "780px" }}>
        {saved ? (
          <div style={{ textAlign: "center", padding: "40px 20px" }}>
            ...
            <h2>Referral Successfully Created!</h2>
            <p style={{ color: "var(--text-secondary)", marginTop: "8px" }}>Opening referral details...</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
            <div>
              <label style={{ display: "block", fontSize: "13px", fontWeight: 600, marginBottom: "6px" }}>Select Patient</label>
              <select
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
              <div>
                <label style={{ display: "block", fontSize: "12px", fontWeight: 600, marginBottom: "4px" }}>Destination Facility</label>
                <select
                  value={facilityName}
              ...
              <div>
                <label style={{ display: "block", fontSize: "12px", fontWeight: 600, marginBottom: "4px" }}>Urgency Level</label>
                <select
                  value={urgency}
            ...
            <div>
              <label style={{ display: "block", fontSize: "12px", fontWeight: 600, marginBottom: "4px" }}>Specialty / Service Required</label>
              <input
                type="text"
                value={specialtyRequired}
                onChange={(e) => setSpecialtyRequired(e.target.value)}
                placeholder="e.g. Obstetrics (ANC Ultrasound), Internal Medicine (Hypertension)"
                style={{ width: "100%", padding: "9px", borderRadius: "8px", border: "1px solid var(--border-color)", background: "var(--card-bg)", color: "var(--text-primary)" }}
              />
            </div>

            <div>
              <label style={{ display: "block", fontSize: "12px", fontWeight: 600, marginBottom: "4px" }}>Reason for Referral</label>
              <textarea
                rows="3"
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                required
                placeholder="Describe clinical findings, high vitals, symptoms, or tests required..."
                style={{ width: "100%", padding: "9px", borderRadius: "8px", border: "1px solid var(--border-color)", background: "var(--card-bg)", color: "var(--text-primary)" }}
              />
            </div>

            <div>
              <label style={{ display: "block", fontSize: "12px", fontWeight: 600, marginBottom: "4px" }}>ASHA Instructions & Field Notes</label>
              <textarea
                rows="2"
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                placeholder="e.g. Advised patient to attend Wednesday morning OPD, transport assistance..."
                style={{ width: "100%", padding: "9px", borderRadius: "8px", border: "1px solid var(--border-color)", background: "var(--card-bg)", color: "var(--text-primary)" }}
              />
            </div>
            ...
            >
              Generate Referral & Notify Facility
            </button>
          </form>
        )}
      </div>
